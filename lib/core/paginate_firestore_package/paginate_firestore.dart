...
class _PaginateFirestoreState extends State<PaginateFirestore> {
    ...
    return BlocBuilder<PaginationCubit, PaginationState>(
      ...
      builder: (context, state) {
        if (state is PaginationInitial) {
          return _buildWithScrollView(context, widget.initialLoader, true);
        } else if (state is PaginationError) {
          return _buildWithScrollView(
              context,
              (widget.onError != null)
                  ? widget.onError!(state.error)
                  : ErrorDisplay(exception: state.error), true);
        } else {
  ...
  Widget _buildWithScrollView(BuildContext context, Widget child, [bool isError= false, double? height]) {
    return isError
        ? Container(
